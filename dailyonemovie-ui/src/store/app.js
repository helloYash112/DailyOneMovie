 ...
 * @param {function} onProgress is callback function basically i used for update progress
  ...
  return fileChunks.map((chunk, i) => {
    ...
    return async () => {
      ...
      const overallPercentage = Math.round(
        (uploadedCount / fileChunks.length) * 100,
      );
...
export function generateSafeFileName(title, movieFileName) {
  // Step 1: sanitize the title (remove unsafe characters)
  const safeTitle = title.replace(/[^a-zA-Z0-9-_]/g, "_");

  // Step 2: extract the extension from the original filename
  const extension = movieFileName.split(".").pop();

  // Step 3: build the final filename
